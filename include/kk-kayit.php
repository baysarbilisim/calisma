...
<?php if(!empty($_SESSION["sepet"]) && !empty($_SESSION["odemetipi"]) && $_SESSION["odemetipi"]>0 && $_SESSION["odemetipi"]<3){ 
	...
	if ($uyebilgisi!=false) {
					...
					$urunStokDusme=$VT->SorguCalistir("UPDATE urunler","SET stok=? WHERE ID=?",array(($urunbilgisi[0]["stok"]-$secenekAdet["adet"]),$urunbilgisi[0]["ID"]),1);
							...
							else
							{
								...
								$toplamFiyat=($fiyat*$bilgi["adet"]);

					$siparisurunEkle=$VT->SorguCalistir("INSERT INTO siparisurunler","SET uyeID=?, siparisID=?, urunID=?, uruntutar=?, adet=?, kdvdurum=?, kdvoran=?, toplamtutar=?, tarih=?",array($uyebilgisi[0]["ID"],$siparisID,$urunbilgisi[0]["ID"],number_format($fiyat,2,".",""),$bilgi["adet"],2,$urunbilgisi[0]["kdvoran"],number_format($toplamFiyat,2,".",""),date("Y-m-d")));
...
<meta http-equiv="refresh" content="0;url=<?=SITE?>gizlilik-politikasi/uyelik-sozlesmesi">

<?php }

	 }else{?>
<meta http-equiv="refresh" content="0;url=<?=SITE?>kurumsal/hakkimizda">
	<?php } ?>
	<!--/main-->