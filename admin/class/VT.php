...
class VT{
	...
	var $user="u294292221_yonetimpaneli";
	var $password="Bk/g?Uu|O6";
	var $dbname="u294292221_yonetimpaneli";
	var $baglanti;
	...
	public function kategoriGetir2($tablo,$sef="",$secID="",$uz=-1){
		...
		if($kategoriler!=false){
			for($x=0;$x<count($kategoriler);$x++){
				...
				if($USTKATID!=false && $USTKATID==$kategoriID){$seciliyap=' selected="selected"';}
				else{$seciliyap='';}
	 ...
	 public function MailGonder($mail,$konu="",$mesaj)
	   {
				...
				$posta->Host     = "smtp.hostinger.com"; // SMTP servers
				$posta->SMTPAuth = true;     // turn on SMTP authentication
				$posta->Username = "info@microtechofficial.com";  // SMTP username
				$posta->Password = "inSide123"; // SMTP password
				$posta->Port     = 587; 
				$posta->From     = "info@microtechofficial.com"; // smtp kullanýcý adýnýz ile ayný olmalý
				$posta->Fromname = "info@microtechofficial.com";
				$posta->AddAddress($mail, "info@microtechofficial.com");
				$posta->Subject  =  $konu;
				$posta->Body     =  $mesaj;
