     ...
     <div class="container-fluid">
         ...
         if($_POST)
         {
             if(!empty($_POST["kategori"]) && !empty($_POST["baslik"]) && 
                !empty($_POST["anahtar"]) && !empty($_POST["description"]) && !empty($_POST["sirano"]) && !empty($_POST["urunkodu"]) && !empty($_POST["fiyat"]) && !empty($_POST["kdvoran"]) && !empty($_POST["genelstok"]) && 
                !empty($_FILES["resim"]["name"]))
             {
                ...
                //$kdvdurum=$VT->filter($_POST["kdvdurum"]);
                 ...
                 if($yukle!=false)
                 {
                     ...
                     {
                         $ekle=$VT->SorguCalistir("INSERT INTO urunler","SET baslik=?, seflink=?, kategori=?, metin=?, urunkodu=?, resim=?, anahtar=?, description=?, fiyat=?, kurus=?, indirimlifiyat=?, indirimlikurus=?, kdvoran=?, kdvdurum=?, stok=?, durum=?, sirano=?, tarih=?",array($baslik,$seflink,$kategori,$metin,$urunkodu,$yukle,$anahtar,$description,$fiyat,$kurus,$indirimlifiyat,$indirimlikurus,$kdvoran,1,$stok,1,$sirano,date("Y-m-d")));
                     }
                     else
                     {
                         $ekle=$VT->SorguCalistir("INSERT INTO urunler","SET baslik=?, seflink=?, kategori=?, metin=?, urunkodu=?, resim=?, anahtar=?, description=?, fiyat=?, kurus=?, kdvoran=?, kdvdurum=?, stok=?, durum=?, sirano=?, 
                            tarih=?",array($baslik,$seflink,$kategori,$metin,$urunkodu,$yukle,$anahtar,$description,$fiyat,$kurus,$kdvoran,1,$stok,1,$sirano,date("Y-m-d")));
                     }
                 ...
                 if($ekle!=false)
                 {
                     if(!empty($_SESSION["varyasyonlar"]) && !empty($_SESSION["secenekler"]))
                     {
                         foreach ($_SESSION["varyasyonlar"] as $value) {
                             ...
                             $varyasyonEkle=$VT->SorguCalistir("INSERT INTO urunvaryasyonlari","SET urunID=?, 
                                baslik=?, tarih=?",array($urunID,$varyasyonAdi,date("Y-m-d")));
                             foreach ($_SESSION["secenekler"][$varyasyonAdi] as $secenek) {
                                 ...
                                 $secenekEkle=$VT->SorguCalistir("INSERT INTO urunvaryasyonsecenekleri","SET 
                                    urunID=?, varyasyonID=?, baslik=?, tarih=?",array($urunID,$varyasyonID,$secenekAdi,date("Y-m-d")));
                             }
                         }
                         ...
                         switch ($varyasyonAdet) {
                             ...
                             {
                                 $varaysyonIDOgreneme=$VT->VeriGetir("urunvaryasyonlari","WHERE baslik=? AND 
                                    urunID=?",array($varaysyon,$urunID));
                                 $secenekIDOgreneme=$VT->VeriGetir("urunvaryasyonsecenekleri","WHERE baslik=? 
                                    AND urunID=?",array($_SESSION["secenekler"][$varaysyon][$w],$urunID));
                             ...
                             for($w=0;$w<count($_SESSION["secenekler"][$varaysyon]);$w++)
                             {
                                 ...
                                 {
                                     $varaysyonIDOgreneme2=$VT->VeriGetir("urunvaryasyonlari","WHERE baslik=? AND 
                                        urunID=?",array($varaysyon2,$urunID));
                                     $secenekIDOgreneme2=$VT->VeriGetir("urunvaryasyonsecenekleri","WHERE baslik=? 
                                        AND urunID=?",array($_SESSION["secenekler"][$varaysyon2][$r],$urunID));
                                        ...
                                        $varaysyonIDOgreneme2!=false && $secenekIDOgreneme2!=false)
                                     {
                                         ...
                                         $genelStokToplami=($genelStokToplami+$varyasyonStok);
                                         $stokNo++;
                                         $stokEkle=$VT->SorguCalistir("INSERT INTO urunvaryasyonstoklari","SET urunID=?, 
                                            varyasyonID=?, secenekID=?, stok=?, 
                                            tarih=?",array($urunID,$varaysyonIDOgreneme[0]["ID"]."@".$varaysyonIDOgreneme2[0]["ID"],$secenekIDOgreneme[0]["ID"]."@".$secenekIDOgreneme2[0]["ID"],$varyasyonStok,date("Y-m-d")));
                                     }
                                 }
                         ...
                         $urunStokGuncelle=$VT->SorguCalistir("UPDATE urunler","SET stok=? WHERE 
                            ID=?",array($genelStokToplami,$urunID),1);
                     }
                     ?>
                         ...
                         <div class="col-md-12">
                             <div class="form-group">
                                 ...
                                 <select class="form-control" name="kdvoran">
                                     ...
                                     <option selected="select" value="8">%8</option>
